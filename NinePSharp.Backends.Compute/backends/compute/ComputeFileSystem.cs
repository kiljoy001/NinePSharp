...
public class ComputeFileSystem : INinePFileSystem
{
    ...
    private static readonly ConcurrentDictionary<string, WasmJob> _globalJobs = new();

    public bool DotU { get; set; }

    public ComputeFileSystem(ComputeBackendConfig config)
    {
        _config = config;
    }
    ...
    public Task<Rwalk> WalkAsync(Twalk twalk)
    {
        ...
        foreach (var name in twalk.Wname)
        {
            ...
            else
            {
                if (tempPath.Count == 0)
                {
                    if (name != "status" && name != "jobs") return Task.FromResult(new Rwalk(twalk.Tag, qids.ToArray()));
                }
                else if (tempPath[0] == "jobs")
                {
                    if (tempPath.Count == 1)
                    {
                        // Allow walking into any job ID
                    }
                    else if (tempPath.Count == 2)
                    {
                        if (name != "code.wasm" && name != "status" && name != "output")
    ...
    private bool IsDirectory(List<string> path)
    {
        ...
        if (path.Count == 2 && path[0] == "jobs") return true; // /jobs/<id>
        return false;
    }
    ...
    public async Task<Rread> ReadAsync(Tread tread)
    {
        ...
        // Console.WriteLine($"READ: Path={string.Join("/", _currentPath)}");
        ...
        {
            data = Encoding.UTF8.GetBytes($"Compute Grid Node\nMax Jobs: {_config.MaxConcurrentJobs}\nActive Jobs: {_globalJobs.Count}\n");
        }
        else if (_currentPath.Count == 3 && _currentPath[0] == "jobs")
        {
            ...
            if (_globalJobs.TryGetValue(id, out var job))
            {
                ...
                else if (file == "output") data = Encoding.UTF8.GetBytes(job.Output.ToString());
            }
        }
    ...
    private byte[] BuildDirectoryListing()
    {
        ...
        {
            items.Add(("code.wasm", QidType.QTFILE));
            items.Add(("status", QidType.QTFILE));
            items.Add(("output", QidType.QTFILE));
        }
    ...
    {
        if (_currentPath.Count == 3 && _currentPath[0] == "jobs" && _currentPath[2] == "code.wasm")
        {
            var id = _currentPath[1];
            if (_globalJobs.TryGetValue(id, out var job))
            {
                job.WasmBinary = twrite.Data.ToArray();
                _ = job.RunAsync(); // Fire and forget execution
    ...
    {
        var name = _currentPath.LastOrDefault() ?? "compute";
    ...
    public Task<Rmkdir> MkdirAsync(Tmkdir tmkdir)
    {
        ...
        {
            var job = new WasmJob(tmkdir.Name);
    ...
    {
        var clone = new ComputeFileSystem(_config);
