...
type Rauth =
    ...
    interface ISerializable with
        ...
        member this.Type = MessageTypes.Rauth
        member this.Tag = this.Tag
        member this.WriteTo(span: Span<byte>) =
            let mutable offset = 0
            writeHeaders this.Size this.Tag MessageTypes.Rauth span &offset
            writeQid this.Aqid span &offset

    static member Parse (data: ReadOnlySpan<byte>) =
        let mutable offset = 0
        let size, tag = readHeaders data &offset
        let aqid = readQid data &offset
        { Size = size; Tag = tag; Aqid = aqid }
