...
public readonly struct Tlock : ISerializable
{
    public uint Size { get; }
    public MessageTypes Type => MessageTypes.Tlock;
    ...
    public Tlock(uint size, ushort tag, uint fid, byte lockType, uint flags, ulong start, ulong length, uint procId, string clientId)
    {
        Size = size;
        Tag = tag;
        Fid = fid;
    ...
    public Tlock(ReadOnlySpan<byte> data)
    {
    ...
    {
        span.WriteHeaders(Size, Tag, MessageTypes.Tlock);
        int offset = NinePConstants.HeaderSize;
