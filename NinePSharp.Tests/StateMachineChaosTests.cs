...
public class StateMachineChaosTests
{
    ...
    public async Task Read_Without_Attach_Returns_Error()
    {
        ...
        var response = await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTread(tread), false);
    ...
    public async Task Double_Clunk_Returns_Error()
    {
        ...
        // 1. Attach
        await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTattach(new Tattach(1, 1, uint.MaxValue, "scott", "chaos")), false);

        // 2. First Clunk
        var tclunk = new Tclunk(1, 1);
        _mockFs.Setup(f => f.ClunkAsync(tclunk)).ReturnsAsync(new Rclunk(1));
        await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTclunk(tclunk), false);

        // 3. Second Clunk (should fail because FID 1 is gone)
        var response = await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTclunk(tclunk), false);
    ...
    public async Task Massive_FID_Usage_Does_Not_Crash()
    {
        ...
        {
            await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTattach(new Tattach(1, i, uint.MaxValue, "scott", "chaos")), false);
        }
        ...
        var response = await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTstat(tstat), false);
