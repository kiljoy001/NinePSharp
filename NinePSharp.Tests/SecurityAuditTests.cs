...
public class SecurityAuditTests
{
    ...
    public async Task Dispatcher_Handles_Fid_Flooding_Stress()
    {
        ...
        for (uint i = 0; i < floodCount; i++)
        {
            ...
            await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTattach(attach), false);
        }

        // Verify we can still use the last one
        var stat = new Tstat(1, floodCount - 1);
        var response = await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTstat(stat), false);
    ...
    public async Task AuthFid_Flooding_Exhaustion_Test()
    {
        ...
        for (uint i = 0; i < floodCount; i++)
        {
            ...
            await dispatcher.DispatchAsync(NinePSharp.Parser.NinePMessage.NewMsgTauth(tauth), false);
        }
