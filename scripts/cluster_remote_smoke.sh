...
dotnet publish "$ROOT_DIR/NinePSharp.Server/NinePSharp.Server.csproj" -c Debug -o "$LOCAL_PACKAGE_DIR" >/dev/null
...
Seed = akka.tcp://${CLUSTER_SYSTEM}@${LOCAL_NODE_HOST}:${LOCAL_NODE_PORT}
Seed = akka.tcp://${CLUSTER_SYSTEM}@${REMOTE_NODE_HOST}:${REMOTE_NODE_PORT}
CONF
...
ssh -p "$REMOTE_PORT" "$REMOTE_SSH" "mkdir -p ${REMOTE_DIR}"
...
ssh -p "$REMOTE_PORT" "$REMOTE_SSH" "cd ${REMOTE_DIR} && if [ -f server.pid ]; then kill \$(cat server.pid) >/dev/null 2>&1 || true; fi && nohup dotnet NinePSharp.Server.dll > server.log 2>&1 & echo \$! > server.pid"
